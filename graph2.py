...
class Graph2():
    def __init__(self, fileName1, fileName2, fileName3):
        ...
        for i in tqdm(self.numVertices):
            ...
            for j in self.numVertices:
                ...
                self.trace[i][j] = -1
                self.timeAll[i][j] = []
                self.timeFastest[i][j] = [self.INF, 0, 0]
                self.pathAll[i][j] = []
                self.cnt[i][j] = 0
        ...
        for routeVarObject in tqdm(varQuery.routeVarList):
            ...
            for stop in stops:
                ...
                if time < self.timeFastest[startStopId][endStopId][0]:
                    self.timeFastest[startStopId][endStopId] = [time, routeId, routeVarId]
    ...
    def topTenImpoStopsDijkstra(self):
        self.dijkstraAll()
    ...
    def topTenImpoStopsDesopoPage(self):
        self.desopoPageAll()
        self.countImportantStops()
        topTen = sorted(self.impo.items(), key = lambda x: x[1], reverse = True)[:10]
        try:
